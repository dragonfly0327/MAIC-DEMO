...
# Import BaseProjectManagementPanel from local base_panel
...
class BOMProjectManagementPanel(BaseProjectManagementPanel):
    ...
    def view_workflow(self, rfq_id, customer):
        ...
        if not os.path.exists(filepath):
            cand1 = os.path.normpath(os.path.join(self.bom_data_dir, customer, f"{rfq_id}.json"))
            cand2 = os.path.normpath(os.path.join(self.bom_data_dir, f"{rfq_id}.json"))
            cand3 = os.path.normpath(os.path.join(self.bom_data_dir, f"{rfq_id.replace(' ', '_')}.json"))
        ...
        if not os.path.exists(filepath):
            safe_rfq = str(rfq_id).strip().lower().replace(" ", "_")
            for root_dir, dirs, files in os.walk(self.bom_data_dir):
                for filename in files:
                    if filename.endswith(".json") and not filename.endswith("metadata.json"):
                        f_lower = filename.lower().replace(" ", "_")
                        if safe_rfq in f_lower or filename.replace(".json", "").lower() == safe_rfq:
        ...
        if not os.path.exists(filepath):
            messagebox.showwarning("File Not Found", f"BOM data file not found for RFQ '{rfq_id}'.\n\nSearched path:\n{filepath}", parent=self)
            return
        ...
        try:
            while step > 0:
                if step == 1:
                    ...
                    win.title(f"View BOM Verification — RFQ {rfq_id} ({customer})")
                    ...
                    win.title(f"View MOQ Assignation — RFQ {rfq_id} ({customer})")
                    ...
                    for assy in data.get("Assemblies", []):
                        ...
                        assembly_moqs[a_num] = [int(x) for x in data.get("Assigned MOQs", [])]
            ...
            print(f"[BOM PM] Error during view workflow step loop: {ex}")
            messagebox.showerror("View Error", f"An error occurred while viewing BOM details:\n{ex}", parent=self)
        finally:
            if win and win.winfo_exists():
                win.destroy()
...
def open_project_management_panel(parent_window, bom_data_dir, module_name=''):
    ...
    panel = BOMProjectManagementPanel(
        win, bom_data_dir,
        title_prefix=prefix,
        user_name=getattr(parent_window, 'user_name', None),
        user_role=getattr(parent_window, 'user_role', None),
        module_context='bom'
    )
