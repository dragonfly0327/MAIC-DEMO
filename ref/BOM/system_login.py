...
try:
    ...
    def get_user_email(name): return "aitinkteng01@gmail.com"

def load_server_role_permissions():
    """Load role permissions dynamically from central server security folder via config.ini."""
            ...
            try:
                ...
                if 'Network' in cfg and 'ServerPath' in cfg['Network']:
                    ...
                    if sp:
                        server_path = sp
                        break
                elif 'PATHS' in cfg and 'server_path' in cfg['PATHS']:
                    sp = cfg['PATHS']['server_path'].strip()
                    if sp:
                        server_path = os.path.normpath(os.path.join(sp, "..", "..", "ContXpps"))
                        break
            except Exception:
                pass

    if not server_path:
        server_path = os.path.normpath(os.path.join(b_dir, "..", "..", "ContXpps"))

    candidate_files = [
        os.path.join(server_path, "security", "role_permissions.json"),
        os.path.normpath(os.path.join(server_path, "..", "ContXpps", "security", "role_permissions.json"))
    ]
...
def verify_launcher_authorization(module_code):
    ...
    4. The user's role has explicit permission to launch `module_code` based on server security role_permissions.json.
    ...
    # Check 2: Server Role Permission Enforcement
    user_role = session_data.get("role", "User")
    username = session_data.get("username", "Unknown")

    server_perms = load_server_role_permissions()
    allowed_list = server_perms.get(user_role, session_data.get("allowed_modules", []))
    allowed_modules = [str(m).lower() for m in allowed_list]

    if user_role.lower() not in ("admin", "system administrator", "top management"):
        if allowed_modules and module_code.lower() not in allowed_modules:
            _show_access_denied_and_exit(
...
def sync_user_directory_email(username, role, email):
    ...
    try:
        ...
        server_path = os.environ.get("CONTXS_SERVER_PATH", "").strip()
                ...
                try:
                    with open(session_file, 'r', encoding='utf-8') as sf:
                        ...
                        if sp and os.path.exists(sp):
                            server_path = os.path.normpath(sp)
                except Exception:
                    pass
        ...
        if not server_path:
            server_path = os.path.normpath(os.path.join(base_dir, "..", "ContXpps"))
